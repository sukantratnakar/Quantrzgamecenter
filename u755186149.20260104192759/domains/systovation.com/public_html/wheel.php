...
<head>
    ...
    .left-panel {
        margin-left: 5%; /* Move closer to center */
        
    }

    .right-panel {
        margin-right: 5%; /* Move closer to center */
...
<body>
    ...
    <div class="wheel-container">
        <header><h1>Roulette Wheel</h1></header>
        ...
        <div id="popup" class="popup">
            <img id="challenge-card" src="" alt="Challenge Card">
            <div class="popup-content">
                ...
                <div class="action-buttons hidden">
                    <button class="action-button" onclick="chooseOption('A')">A</button>
                    <button class="action-button" onclick="chooseOption('B')">B</button>
                    <button class="action-button" onclick="chooseOption('C')">C</button>
                    <button class="action-button" onclick="chooseOption('D')">D</button>
                ...
                <div class="slider-input">
                    <p id="winLabel1">How would you rate this answer: </p><p id="winLabel">0</p>
                    <input type="range" min='0' id="reward-slider">
                    <button class="popup-button" id="wildAnswerButton" onclick="wildAnswer()">Show Notes</button>
                    <button class="popup-button" id="wildAnswerButton1" onclick="extraTime()">Extra Time</button>
                    <button class="popup-button" id="wildRewardButton" onclick="wildReward()">Reward</button>
                    <button class="doneWildButton" id="doneWildButton" onclick="skipChallenge()">Done</button>
                </div>
                <div class="before-wc">
    ...
    <script>
        ...
        let numbers = localStorage.getItem(userEmail + 'numbers').split(','); //   ['0', '00', ...Array.from({ length: 36 }, (_, i) => (i + 1).toString())];
        ...
        function endGame(){
            window.location.href = 'table.php?type='
        }
        ...
        function displayCurrentPlayer() {
           ...
           playerDATA.innerHTML = `
                <br>
                <h2>Current Round:  <b> ${round}</b></h2>
                <h2>Current Player:  <b> ${data[0]}</b></h2>
                <h2>Current Team:  <b> ${data[1]}</b></h2>
            `;
        ...
        function saveOrder(){
            ...
            localStorage.removeItem(userEmail + "order");
            localStorage.setItem(userEmail + "order", SaveOrder);
        }
       ...
       async function loadCosts() {
        ...
        function playChallenge() {
            ...
            if(team.timeTokens - cards[winningNumber].timeCost >= 0 && team.energyTokens - cards[winningNumber].energyCost >= 0){
                ...
                localStorage.setItem(userEmail + 'numbers', numbers); //   ['0', '00', ...Array.from({ length: 36 }, (_, i) => (i + 1).toString())];
                drawWheel();
            }
            else{
                showAlertPopupInformationDef("Your team doesn't have enough tokens to play! Try trading with other teams.");
                //alert("Your team doesn't have enough tokens to play! Try trading with other teams.");
                return;
            }
            
            challengeCard.style.filter = 'none';
            displayTeamInfo();
            switchShowButtons(1);
        ...
        function chooseOption(option) {
            setCard(false);
            if(option === cards[winningNumber].correctAnswer){ 
                team.coins += cards[winningNumber].winGain;
                team.profit += cards[winningNumber].winGain;
                showAlertPopupInformationDef("Correct answer! You won 10 coins!");
                //alert("Correct answer! You won 10 coins!");
                displayTeamInfo();
            }
            else{
                showAlertPopupInformationDef("Wrong answer! You won 0 coins!");
                //alert("Wrong answer! You won 0 coins!");
            }
            setCard(false);
        ...
        function showWildCard(){
            stopProgressBar();
            startProgressBar(450, false);
            const card = setWildCard(true);
        ...
        function wildReward(){
            ...
            document.getElementById('doneWildButton').style.display = "flex";
        ...
        let display = 0;
        function switchShowButtons(much = -1){
            if(much != -1){
                display = much;
            }
            ...
            switch (display){
                case 0:
                    ...
                    let data = currentPlayer.split(" ");
                    //const text = ("Current player: " + data[0] + " team(" + data[1] + ")");
                    //document.getElementById('current-player-popup').innerHTML = text;
                    console.log(text);
                    break;
                case 1:
        ...
        function extraTime() {
            ...
            if(team.timeTokens - cards[winningNumber].additionalTimeCost >= 0){
                ...
                //alert("Time Token Exhausted: You don’t have enough time tokens to buy extra time!");
            }
        ...
        function displayTeamInfo() {
            ...
            teamsInfoDiv.innerHTML = teams.map(team => `
                ...
                <hr>
            `).join('');
            localStorage.setItem(userEmail + "teams", JSON.stringify(teams));   //coment this line for TESTING
        }

        function setWildCard(front){
            const popup = document.getElementById('popup');
            const challengeCard = document.getElementById('challenge-card');
            document.getElementById('wildRewardButton').style.display = 'none';
            ...
            document.getElementById('doneWildButton').style.display = "none";

            if(front){
                challengeCard.src = `cards/WildCard${winningNumber}.jpg`;
                challengeCard.style.filter = 'blur(5px)';
            }
            else{
                challengeCard.src = `cards/WildCard${winningNumber}A.jpg`;
        ...
        function setCard(front){
            ...
            stopProgressBar();
            if(front){
                challengeCard.src = `cards/ChallengeCard${winningNumber}.jpg`;
                challengeCard.style.filter = 'blur(5px)';
            }
            else{
                challengeCard.src = `cards/ChallengeCard${winningNumber}A.jpg`;
            }
        }
